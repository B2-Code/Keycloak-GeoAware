...
GeoAware is configured through Keycloak's standard SPI configuration mechanism.
You can set options in any of the following ways:

**`keycloak.conf`:**

```
spi-geoaware-geoip--provider=ipinfo-webservice
```

**Environment variable** (uppercase, dashes replaced with underscores, double dashes become double underscores):

```
KC_SPI_GEOAWARE_GEOIP__PROVIDER=ipinfo-webservice
```

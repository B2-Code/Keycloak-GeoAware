...
import org.b2code.geoip.provider.maxmind.MaxmindFileAutodownloadProviderFactory;
import org.b2code.geoip.provider.maxmind.MaxmindFileProviderFactory;
import org.b2code.geoip.provider.maxmind.MaxmindWebServiceProviderFactory;
...
public class MockGeoipProviderFactory implements GeoIpProviderFactory {
    ...
    public GeoIpProvider create(KeycloakSession keycloakSession) {
        ...
        GeoIpProvider provider = factory.create(keycloakSession);

        return Optional.ofNullable(keycloakSession.getContext())
                .map(KeycloakContext::getRealm)
                .map(realm -> realm.getAttribute(PluginConstants.PLUGIN_NAME_LOWER_CASE + "-mock-ip"))
                .map(mockIp -> new DelegatingGeoIpProvider(provider, mockIp))
                .orElseGet(() -> new DelegatingGeoIpProvider(provider, null));
    }
